...
export default function Publications() {
  ...
  return (
    <section 
      ref={sectionRef} 
      id="publications"
      className="relative bg-[#0B0B0D] z-50 py-[8vh] px-4 md:px-[7vw]"
    >
      {/* Header */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        {publications.map((pub, index) => (
          ...
          >
            {/* Border */}
            <div 
              className="card-border absolute inset-0 border border-white/20 pointer-events-none group-hover:border-lime/50 transition-colors"
            ...
            <div className="relative p-6 md:p-8">
              ...
              {/* Title */}
              <h3 className="text-[clamp(18px,5vw,24px)] md:text-[clamp(20px,1.8vw,24px)] font-bold text-[#F4F4F5] mb-4 leading-tight">
                {pub.title}
              </h3>
              
              {/* Abstract */}
              <p className="text-[#A7A7AA] text-sm md:text-base leading-relaxed mb-6">
                {pub.abstract}
              </p>
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {pub.tags.map((tag, tagIndex) => (
                  <span 
                    key={tagIndex}
                    className="px-2 md:px-3 py-1 border border-white/20 text-[#A7A7AA] font-mono text-xs uppercase tracking-wider"
                  >
                    {tag}
                  </span>
                ))}
              ...
              <a 
                ...
                className="group/link inline-flex items-center gap-2 text-[#F4F4F5] font-mono text-sm uppercase tracking-[0.14em] hover:text-lime transition-colors"
              >
                View DOI
